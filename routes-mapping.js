...
module.exports = mapApp = (req, map) => {
  for (let i = 0; i < req.app._router.stack.length; i++) {
    ...
    if (req.app._router.stack[i].route) {
      ...
      if (!map[req.app._router.stack[i].route.path]) map[req.app._router.stack[i].route.path] = {};
      ...
      req.app._router.stack[i].route.stack.forEach((element, index) => {
        ...
        map[req.app._router.stack[i].route.path][method + '_functions'].push(name.length > 0 ? name : "anonymous")
